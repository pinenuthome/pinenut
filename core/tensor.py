...
class Tensor:
    ...
    def backward(self, retain_grad=False, create_graph=False):
        ...
        :param create_graph: whether to build a graph for gradient computation
        '''
    ...
    def device(self):
        ...
        if xp is np:
            return 'cpu'
        return 'cuda:{}'.format(self.data.device.id)
    ...
    def cuda(self, device=None):
        device_id = cuda._device_index(device)
        if self.data is not None:
            self.data = cuda.as_cupy(self.data, device=device_id)
        if self.grad is not None:
            self.grad.cuda(device_id)
        self._device = 'cuda:{}'.format(device_id)
        return self
...
def tensor(data, dtype=None, name=None):
    return Tensor(as_array(data, dtype=dtype), name=name)
...
class Operator:
    ...
    def _do_forward(self, *inputs):
        ...
        outputs = [as_array(a, xp=cuda.get_array_module(a)) for a in raw_output]
...
def add(x0, x1):
    x1 = as_array(x1, xp=cuda.get_array_module(x0.data))
    return Add()(x0, x1)
...
class Transpose(Operator):
    ...
    def backward(self, grad_output):
        ...
        if inv_axes:
            ...
            xp = cuda.get_array_module(self.inputs[0].data)
...
class MatMul(Operator):
    ...
    def backward(self, grad_output):
        ...
        a_matrix = expand_dims(a, -2) if a.ndim == 1 else a
        b_matrix = expand_dims(b, -1) if b.ndim == 1 else b
        grad_matrix = grad_output
        if a.ndim == 1:
            grad_matrix = expand_dims(grad_matrix, -2)
        if b.ndim == 1:
            grad_matrix = expand_dims(grad_matrix, -1)

        def swap_last_two(value):
            axes = list(range(value.ndim))
            axes[-2], axes[-1] = axes[-1], axes[-2]
            return transpose(value, tuple(axes))

        grad_a = matmul(grad_matrix, swap_last_two(b_matrix))
        grad_b = matmul(swap_last_two(a_matrix), grad_matrix)
        grad_a = sum_to(grad_a, a_matrix.shape).reshape(a.shape)
        grad_b = sum_to(grad_b, b_matrix.shape).reshape(b.shape)
...
class Index(Operator):
    ...
    def backward(self, grad_output):
        xp = cuda.get_array_module(self.inputs[0])
...
class Concat(Operator):
    ...
    def backward(self, grad_output):
        ...
        xp = cuda.get_array_module(xs[0])
        grads = xp.split(grad_output.data, split_points, axis=axis)
        return tuple(as_tensor(grad) for grad in grads)
